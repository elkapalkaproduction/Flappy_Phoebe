...
public:
	...
	bool Update(float dt)
	{
		...
		else if (_parent->GetState() == FlappyGameplayState::Running)
		{
			...
			if (_nextWallToPass != nullptr && _nextWallToPass->IsCrocodilePassWall(_crocodile->GetBoundingBox()))
			{
                ...
                SoundManager::Inst().PauseGameMusic();
			...
			if (IsCrocodileCollisioningGround() || IsCrocodileCollisioningWalls())
			{
				...
				if (IsCrocodileCollisioningGround())
				{
                    ...
                    SoundManager::Inst().PlaySoundGround();
				}
                else
                {
	...
	{
#ifndef SK_PAID
        //sk::game_services::show_revmob_banner(true);
#endif
...
private:
	...
	void UpdateCrocodileParticles(float dt, float dx)
	{
		...
		{
			_lastParticleTime = 0;	
			auto sprite = SpriteHelper::CreateSpriteWithTextureName(this, 3, "textures/particle.png");
...
//    ig_interface::Button* _butFacebook;
//    ig_interface::Button* _butTwitter;
	FlappyGameplayLayer* _parent;
...
//        _butTwitter(),
	...
	{
		auto winSize = InputHelper::GetUnscaledWindowSize();
        std::string path = "textures/ecran/";
        path.append(CCUserDefault::sharedUserDefault()->getStringForKey("AVPreferedLanguage"));

        auto spriteBckg = SpriteHelper::CreateSpriteWithTextureName(this, 0, "textures/ecran/fon-B.png");
		...
		auto spriteGameOver = SpriteHelper::CreateSpriteWithTextureName(this, 0, "textures/ecran/ENG-game-over.png");
		spriteGameOver->getTexture()->setAliasTexParameters();
		spriteGameOver->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.86f));
		spriteGameOver->setScale(0.68f);
        
		auto spriteBestPanel = SpriteHelper::CreateSpriteWithTextureName(this, 0, "textures/ecran/ENG-best.png");
		spriteBestPanel->getTexture()->setAliasTexParameters();
		spriteBestPanel->setPosition(ccp(winSize.width * 0.64f, winSize.height * 0.7975f));
		spriteBestPanel->setScale(0.68f);
        
		auto spriteScorePanel = SpriteHelper::CreateSpriteWithTextureName(this, 0, "textures/ecran/ENG-score.png");
		spriteScorePanel->getTexture()->setAliasTexParameters();
		spriteScorePanel->setPosition(ccp(winSize.width * 0.35f, winSize.height * 0.7922f));
		spriteScorePanel->setScale(0.68f);

		_labMaxScore = CCLabelWithShadow::create("0", "fonts/hoog0555.fnt", ccp(3, -3), kCCTextAlignmentCenter);
		addChild(_labMaxScore, 0);
//		_labMaxScore->SetLabelAnchorPoint(ccp(0.5f, 0.5f));
		_labMaxScore->setScale(0.7f);
		_labMaxScore->setColor(ccc3(128, 200, 200));
		_labMaxScore->setPosition(ccp(spriteBestPanel->getPositionX() + 60,spriteBestPanel->getPositionY()));
        
		_labScore = CCLabelWithShadow::create("0", "fonts/hoog0555.fnt", ccp(3, -3), kCCTextAlignmentCenter);
		addChild(_labScore, 0);
//		_labScore->SetLabelAnchorPoint(ccp(0.5f, 0.5f));
		_labScore->setScale(0.7f);
		_labScore->setColor(ccc3(128, 200, 200));
		_labScore->setPosition(ccp(spriteScorePanel->getPositionX() + 60, spriteScorePanel->getPositionY()+3));

		//spriteScorePanel
		//

		_butPlay = AddCoreButton(this, this, path+"-Play.png");
		_butPlay->setPosition(ccp(winSize.width*0.78, 60));
        
        _butRate = AddCoreButton(this, this, path+"-Rate.png");
		_butRate->setPosition(ccp(winSize.width*0.57, winSize.height * 0.50f));

        _butMoreGames = AddCoreButton(this, this, path+"-More-Games-B.png");
		_butMoreGames->setPosition(ccp(winSize.width*0.57, winSize.height * 0.40f));
        
        _butRange = AddCoreButton(this, this, path+"-Who-are-Neoniks-B.png");
		_butRange->setPosition(ccp(winSize.width*0.57, winSize.height * 0.30f));

//        QFunction<void(bool)> response;
//        CCMessageBoxYesNo::ShowWithBool("fuck this shit", "", "dsaa", "sas", response)
        
        
//        _butFacebook = AddCoreButton(this, this, "textures/gui/b_facebook.png");
//		_butFacebook->setPosition(ccp(winSize.width*0.74, winSize.height * 0.06f));
//        
//        _butTwitter = AddCoreButton(this, this, "textures/gui/b_twitter.png");
//		_butTwitter->setPosition(ccp(winSize.width*0.89, winSize.height * 0.06f));
	...
	void SetActive(bool isActive)
	{
		if (isActive)
		{
            ...
            _butRate->registerWithTouchDispatcher();
            _butMoreGames->registerWithTouchDispatcher();
//            _butFacebook->registerWithTouchDispatcher();
//            _butTwitter->registerWithTouchDispatcher();
		}
		else
		{
			_butPlay->unregisterWithTouchDispatcher();
			_butRange->unregisterWithTouchDispatcher();
            _butRate->unregisterWithTouchDispatcher();
            _butMoreGames->unregisterWithTouchDispatcher();
//            _butFacebook->unregisterWithTouchDispatcher();
//            _butTwitter->unregisterWithTouchDispatcher();
		}
	...
	void SetScore(int score)
	{
		...
		if (score > maxScore)
		{
            std::string lang =CCUserDefault::sharedUserDefault()->getStringForKey("AVPreferedLanguage");
            if (lang.compare("RUS") == 0) {
                sk::game_services::open_yes_no_dialog(0, 0, "Поздравлаю с результатом!  Вот бесплатная игра в подарок:","","Да","Нет");
                
            } else {
                sk::game_services::open_yes_no_dialog(0, 0, "Congrats on high score!  As a prize, here's a free game:","","Yes","No");
            }
...
private:
	virtual void onClickAction(Button* const target) override
	{
		if (target == _butPlay)
		{
            SoundManager::Inst().ResumeGameMusic();
...
//        else if (target == _butFacebook)
//        {
//            static const char* maxScoreString = "sdkghpPPasSS";
//            int maxScore = CCUserDefault::sharedUserDefault()->getIntegerForKey(maxScoreString, 0);
//            
//            char scoreStr[1024];
//            sprintf(scoreStr, "Your max score is: %d", maxScore);
//            sk::game_services::share_game_score_facebook(scoreStr);
//        }
//        else if (target == _butTwitter)
//        {
//            static const char* maxScoreString = "sdkghpPPasSS";
//            int maxScore = CCUserDefault::sharedUserDefault()->getIntegerForKey(maxScoreString, 0);
//            
//            char scoreStr[1024];
//            sprintf(scoreStr, "Your max score is: %d", maxScore);
//            sk::game_services::share_game_score_twitter(scoreStr);
//        }
	}
...
private:
	...
	//ig_interface::Button* _butRate;
	ig_interface::Button* _butMoreGames;
    ig_interface::Button* _butAds;
	ig_interface::Button* _butRestore;
...
public:
	...
	void Init()
	{
		...
		auto winSize = InputHelper::GetUnscaledWindowSize();
        auto spriteBckg = SpriteHelper::CreateSpriteWithTextureName(this, 0, "textures/ecran/fon-A.png");
		spriteBckg->getTexture()->setAliasTexParameters();
		spriteBckg->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.5f));
        if (CCUserDefault::sharedUserDefault()->getIntegerForKey("AVdeviceType") == 1) {
            spriteBckg->setScale(0.9f);
        ...
        auto spriteText = SpriteHelper::CreateSpriteWithTextureName(this, 0, path+"-text.png");
		spriteText->getTexture()->setAliasTexParameters();
		spriteText->setPosition(ccp(winSize.width * 0.51f, winSize.height * 0.65f));
		spriteText->setScale(0.68f);
 
		_spriteTitle = SpriteHelper::CreateSpriteWithTextureName(this, 0, path+"-title.png");
		_spriteTitle->getTexture()->setAliasTexParameters();
		_spriteTitle->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.82f));
		_spriteTitle->setScale(0.68f);

		_butPlay = FlappyGameOverLayer::AddCoreButton(this, this, path+"-Play.png");
		_butPlay->setPosition(ccp(winSize.width*0.78, 37));


		_butRange = FlappyGameOverLayer::AddCoreButton(this, this, path+"-Who-are-Neoniks-A.png");
		_butRange->setPosition(ccp(winSize.width * 0.5f, winSize.height * 0.14f));

		_butMoreGames = FlappyGameOverLayer::AddCoreButton(this, this, path+"-More-Games-А.png");
		_butMoreGames->setPosition(ccp(winSize.width*0.19, 37));

        
        // PMM - Taking out Ads and Restore
        //_butAds = FlappyGameOverLayer::AddCoreButton(this, this, "textures/gui/b_ads.png");
		//_butAds->setPosition(ccp(winSize.width*0.74, winSize.height*0.06));
        
		//_butRestore = FlappyGameOverLayer::AddCoreButton(this, this, "textures/gui/b_restore.png");
		//_butRestore->setPosition(ccp(winSize.width*0.89, winSize.height*0.06));
...
/*#ifndef SK_PAID
	...
	void SetActive(bool isActive)
	{
		Button* buttons[] = { _butPlay, _butRange, _butMoreGames };
		for (auto button : buttons)
		{
			isActive ? button->registerWithTouchDispatcher() : button->unregisterWithTouchDispatcher();
		}

		CCSprite* sprites[] = { _spriteTitle, GetUpperSprite(_butPlay),
...
//            _parent->Restart(false);
        ...
        // PMM - taking out restore stuff and ads stuff
        //else if (target == _butAds)
		//{
		//	sk::game_services::buy_product(0);
		//}
        //else if (target == _butRestore)
		//{
